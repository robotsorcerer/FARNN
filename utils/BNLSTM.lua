...
function BNLSTM:buildInputGate()
   self.inputGate = self:buildGate()
   return self.inputGate
end

function BNLSTM:buildForgetGate()
...
function BNLSTM:buildOutputGate()
...
------------------------- forward backward -----------------------------
function BNLSTM:updateOutput(input)
   ...
   -- output(t), cell(t) = BNLSTM{input(t), output(t-1), cell(t-1)}
   local output, cell
...
function BNLSTM:_updateGradInput(input, gradOutput)
...
function BNLSTM:_accGradParameters(input, gradOutput, scale)
